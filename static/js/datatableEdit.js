	 ...
	 switch(e.key) {
		 case 'Enter':
			 ...
			 if(editor.originCell.type === 'number') {
				 cur = parseFloat(cur); // Parse as a floating-point number
			 }
 
			 // Check if it's the 2nd column (0-based index)
			 if (editor.editingCellIndex.col === 2) { // 2번째 열 (0부터 시작하므로 1)
				 cur = Math.min(100, Math.max(0, cur)); // Ensure the value is within 0-100 range
			 }
