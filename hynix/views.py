...
def process_file(test_data_file):
    ...
    predictions = get_predictions(test_data_file)
    return predictions
...
def simulation(request):
    ...
    if request.method == "POST":
        ...
        if 'timestamp' in request.POST:
            run_timestamp = request.POST['timestamp']  # Run 버튼 누른 시점의 시간 값을 받아옴
            ...
            if 'isFinish' in request.POST and request.POST["isFinish"] == 1:
...
def lifecycle(request):
    ...
    if request.method == "POST":
        ...
        return JsonResponse({"status": "success", "avg_del": avg_delta})
